...
while line:
    ...
    y_test = []
    for i in range(len(tokens) - 2):  #we need to take tri-grams so -2#
        x_test_1.append([word2idx[tokens[i]]])  # !!!#
        x_test_2.append([word2idx[tokens[i+1]]])  # !!!#

        y_test.append([word2idx[tokens[i + 2]]])  # !!!#

    x_test = np.concatenate((np.array(x_test_1), np.array(x_test_2)), axis=1)
